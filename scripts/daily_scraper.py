...
def extract_vehicle_smart(page: Page, slug: str, category: str) -> Optional[dict]:
    ...
    try:
        page_data = page.evaluate(r"""() => {
            ...
            // FIX: this was preferring the og:image meta tag (a wide
            // 1200x630 social-share banner meant for link previews), then
            // falling back to a ".image img" selector that doesn't match
            // this site's real markup. The actual per-vehicle icon —
            // exactly the kind of image a grid card wants — lives on a
            // dedicated Gaijin asset CDN at a predictable path and is
            // present on every vehicle page right next to the country
            // flag. Preferring that first, with the old logic kept as a
            // fallback chain rather than removed, since it's still a
            // valid signal if the primary selector ever stops matching.
            let imgUrl = null;
            let iconImg = document.querySelector('img[src*="static.encyclopedia.warthunder.com/images/"]');
            if (iconImg && iconImg.src) {
                imgUrl = iconImg.src;
            } else {
                let ogImage = document.querySelector('meta[property="og:image"]');
                imgUrl = ogImage ? ogImage.content : null;
                if (!imgUrl) {
            ...
            // ---- aviation-specific clean single-value fields ----
            // Verified against a real fetched P-51D-5 page. Each of these
            // is a single unambiguous number on the page (unlike max
            // speed / climb rate / power-to-weight, which render as 4
            // concatenated mode values with no separator and stay
            // unresolved on purpose). Harmless null for categories that
            // don't have them (ground/ships).
            const maxAltitudeM = grab(/Max altitude\s+([\d,]+)\s*m\b/);
            const takeoffRunM = grab(/Takeoff Run\s+([\d,]+)\s*m\b/i);
            const lengthM = grab(/\bLength\s+([\d.]+)\s*m\b/);
            const wingspanM = grab(/Wingspan\s+([\d.]+)\s*m\b/);
            const gunAmmoM = grab(/Ammunition\s+([\d,]+)\s*rounds\b/i);
            const fireRateM = grab(/Fire rate\s+([\d,]+)\s*shots\/min/i);

            // ---- flight performance (confirmed via live DevTools
            // inspection, not text-parsing guesswork) ----
            // These render as a page toggle (Realistic/Arcade,
            // Reference/Basic) that swaps ONE clean value in and out of
            // the DOM — at the moment Playwright loads the page (before
            // any click), only the default state's numbers exist in
            // innerText, with none of the ambiguous concatenation seen
            // in an earlier, less reliable text-fetch pass. Labeled
            // honestly as the default-state values, not "the" value —
            // the other 3 toggle states aren't captured since clicking
            // through all 4 combinations for every vehicle would multiply
            // scrape time ~4x for a gain that doesn't change the
            // Realistic-mode number most players actually look up.
            const turnTimeM = grab(/Turn time\s+([\d.]+)\s*s\b/);
            const rateOfClimbM = grab(/Rate of Climb\s+([\d.]+)\s*m\/s/i);
            const maxSpeedM = grab(/Max speed\s+at\s+([\d,]+)\s*m\s+([\d,]+)\s*km\/h/i);
            const baseWeightM = grab(/Base weight\s+([\d.]+)\s*t\b/i);
            const wingLoadingM = grab(/Wing Loading\s+([\d,]+)\s*kg\/m/i);
            ...
            document.querySelectorAll('table').forEach(table => {
                ...
                rows.forEach(row => {
                    ...
                    if (cells.length >= 8) {
                        ...
                        // FIX: the "List of available weapons" table (bombs,
                        // rockets, missiles carried externally) also has
                        // >=8 cells — its extra columns are slot-diagram
                        // icons, not penetration values — so every "range"
                        // cell parses to NaN/null. A bomb has no armor
                        // penetration at range; a row where every
                        // penetration value is null isn't a real
                        // ammunition/belt entry and was previously getting
                        // pushed in anyway with a name and weight but all
                        // dashes, cluttering the Firepower table with
                        // entries like "AIM-9L Sidewinder ... — — — —".
                        const hasRealPenetration = Object.values(pen).some((v) => v !== null);
                        if (rawName && type && hasRealPenetration) ammo.push({ name: rawName, type: type, penetrationMm: pen });
                    }
                });
            });
            ...
            return {
                ...
                maxAltitudeM: maxAltitudeM ? maxAltitudeM[1] : null,
                takeoffRunM: takeoffRunM ? takeoffRunM[1] : null,
                lengthM: lengthM ? lengthM[1] : null,
                wingspanM: wingspanM ? wingspanM[1] : null,
                gunAmmoCount: gunAmmoM ? gunAmmoM[1] : null,
                fireRateRpm: fireRateM ? fireRateM[1] : null,
                turnTimeSec: turnTimeM ? turnTimeM[1] : null,
                rateOfClimbMs: rateOfClimbM ? rateOfClimbM[1] : null,
                maxSpeedKmh: maxSpeedM ? maxSpeedM[2] : null,
                maxSpeedAltitudeM: maxSpeedM ? maxSpeedM[1] : null,
                baseWeightT: baseWeightM ? baseWeightM[1] : null,
                wingLoadingKgm2: wingLoadingM ? wingLoadingM[1] : null,
                ammo: ammo
            };
        }""")
    except Exception as e:
    ...
    # dynamic_specs holds ONLY specifically-verified, unambiguous
    # single-value fields — never a generic "grab whatever's on the page"
    # dump. The two things that used to fill it that way (the old
    # ".infobox tr" selector, and later a generic line-pairing heuristic)
    # both turned out to pick up unrelated page chrome — cookie-consent
    # banner buttons, nav menu items — and shipped that straight to the
    # site's UI as if it were real vehicle data. Every key below has a
    # clear numeric value with an explicit unit boundary (a number
    # followed by "m", "rounds", "%", etc.), confirmed against a real
    # fetched page — not a guess at where free text starts/ends. Missing
    # for a given vehicle (e.g. ground vehicles don't have "Wingspan")
    # just means that key isn't present; never a wrong guess.
    dynamic_specs = {}
    _raw_extras = {
        "visibility_pct": page_data.get("visibilityPct"),
        "max_altitude_m": page_data.get("maxAltitudeM"),
        "takeoff_run_m": page_data.get("takeoffRunM"),
        "length_m": page_data.get("lengthM"),
        "wingspan_m": page_data.get("wingspanM"),
        "gun_ammo_count": page_data.get("gunAmmoCount"),
        "fire_rate_rpm": page_data.get("fireRateRpm"),
        "turn_time_s": page_data.get("turnTimeSec"),
        "rate_of_climb_ms": page_data.get("rateOfClimbMs"),
        "max_speed_kmh": page_data.get("maxSpeedKmh"),
        "max_speed_altitude_m": page_data.get("maxSpeedAltitudeM"),
        "base_weight_t": page_data.get("baseWeightT"),
        "wing_loading_kgm2": page_data.get("wingLoadingKgm2"),
    }
    for key, val in _raw_extras.items():
        if val not in (None, ""):
            dynamic_specs[key] = str(val).replace(",", "")
