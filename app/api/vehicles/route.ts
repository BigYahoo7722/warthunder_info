...
interface VehicleRow {
  ...
  armor: { hull?: ArmorTriplet; turret?: ArmorTriplet; composite?: boolean; era?: boolean } | null;
  ammunition: ScrapedAmmo[] | null;
  research_cost_rp: number | null;
  purchase_cost_sl: number | null;
  // FIX: was collected by the scraper and stored in Supabase, but never
  // selected/mapped here — so the card component had nothing to
  // auto-organize even though the raw label->value data existed all
  // along. Every field the scraper finds on the wiki page (whatever it
  // is, however it's named) lands here as-is.
  // FIX: the scraper has written this column since the very first
  // version and Supabase has always stored it, but it was never
  // selected/typed here — so VehicleCard had nothing to render even
  // though real image URLs existed in the database the whole time.
    ...
    armor:
      row.armor?.hull || row.armor?.turret
        ? {
            ...
            // FIX: daily_scraper.py has extracted these for a while now
            // (armor.composite / armor.era in the jsonb column) — this was
            // still hardcoded false, silently discarding real scraped data.
            era: Boolean(row.armor.era),
            composite: Boolean(row.armor.composite),
          }
        : undefined,
    proTips: [],
    sourceDetail: "scraped",
    imageUrl: row.image_url ?? undefined,
    // FIX: previously dropped entirely — every raw label/value pair the
    // scraper found on the wiki page (whatever it is, named however the
    // wiki names it) now reaches the frontend so the card can organize
    // and display it itself instead of needing a hand-coded field for
    // each one.
    dynamicSpecs: row.dynamic_specs ?? undefined,
  };
}
