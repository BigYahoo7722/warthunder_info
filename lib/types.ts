...
export interface Vehicle {
  ...
  proTips: string[];
  // FIX: the scraper has collected this from the very first version, and
  // Supabase has stored it the whole time, but no type/mapping ever
  // carried it from the database row to the frontend Vehicle shape — so
  // it silently never reached VehicleCard. Hotlinked directly from
  // Gaijin's own asset CDN, never downloaded or re-hosted.
  imageUrl?: string;
  isPremium?: boolean;
  isEvent?: boolean;
  isRare?: boolean;
