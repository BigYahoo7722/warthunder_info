...
import VehicleRevealScene from "./three/VehicleRevealScene";

// Same reasoning as CinematicHero.tsx — Scene3D needs a real browser, so
// it must never enter the server-render pass Next.js does at build time.
const Scene3D = dynamic(() => import("./three/Scene"), { ssr: false });

// Same known-key / auto-categorization logic as the original VehicleModal —
// this is real, working self-organizing logic for whatever fields the
// scraper finds, it just didn't need to change for the visual rebuild.
const KNOWN_KEYS = [
...
const AUTO_SECTIONS: { title: string; eyebrow: string; match: RegExp }[] = [
  ...
  { title: "Mobility", eyebrow: "MOBILITY", match: /speed|engine|power|weight|turn|climb|transmission|accel|mobility|altitude|takeoff|length|wingspan|displacement|loading/i },
  { title: "Firepower", eyebrow: "ARMAMENT", match: /ammo|penetrat|reload|gun|cannon|rocket|missile|bomb|weapon|magazine|caliber|fire.?rate/i },
...
function formatLabel(rawKey: string): string {
  return rawKey.replace(/_/g, " ").replace(/([a-z])([A-Z])/g, "$1 $2").trim().replace(/^./, (c) => c.toUpperCase());
}
...
export function VehicleDossierModal({
  vehicle,
  onClose,
  translateEnabled,
}: {
  ...
  const unknownTopLevel = Object.fromEntries(
    Object.keys(vehicle)
      .filter((key) => !KNOWN_KEYS.includes(key) && (vehicle as any)[key] !== null && (vehicle as any)[key] !== undefined)
      .map((key) => [key, (vehicle as any)[key]])
  );
  ...
  const orderedGroups = sectionOrder.filter((title) => grouped.has(title)).map((title) => [title, grouped.get(title)!] as const);
  ...
  return (
    <AnimatePresence>
      <motion.div
        ...
        role="dialog"
      >
        {/* ---- Cinematic reveal header: replaces the flat gradient plate ---- */}
        <div className="relative h-[46vh] min-h-[320px] w-full shrink-0 overflow-hidden border-b border-hairline">
          <Scene3D cameraPosition={[-4, 2.6, 9]}>
            <VehicleRevealScene key={vehicle.id} vehicle={vehicle} autoplay />
          </Scene3D>

          <button
            type="button"
            onClick={onClose}
            className="absolute end-3 top-3 z-20 flex h-9 w-9 items-center justify-center rounded-full border border-hairline bg-ink/60 backdrop-blur font-mono text-parchment/70 hover:border-brass hover:text-brass"
          >
            ✕
          </button>

          <div className="pointer-events-none absolute inset-x-0 bottom-0 z-10 bg-gradient-to-t from-ink via-ink/40 to-transparent p-4 pt-16 sm:p-8 sm:pt-24">
            <p className="font-mono text-[10px] uppercase tracking-widest2 text-brass">
              {t(`nation.${vehicle.nation}`)} · Rk {vehicle.rank}
            </p>
            <h2 className="mt-1 font-display text-3xl tracking-wide text-parchment sm:text-4xl">
              {displayName}
            </h2>
          </div>
        </div>
        ...
        {/* ---- BR strip ---- */}
        <div className="grid shrink-0 grid-cols-3 divide-x divide-hairline border-b border-hairline bg-panel/80 backdrop-blur">
        ...
        {/* ---- Full dossier body — same data density as before, glass surfaces ---- */}
        <div className="mx-auto w-full max-w-3xl flex-1 space-y-4 p-4 sm:p-6">
          {(vehicle.crew || vehicle.mobility?.weightTons) && (
            <div className="grid grid-cols-2 gap-3 rounded-xl border border-hairline bg-panel/60 p-3 font-mono text-xs backdrop-blur sm:grid-cols-3">
              {vehicle.crew ? (
                <div className="flex flex-col border-l-2 border-brass/30 pl-2">
                  ...
                  <span className="mt-0.5 text-sm font-bold text-parchment"><bdi dir="ltr">{vehicle.crew}</bdi></span>
                </div>
              ) : null}
              {vehicle.mobility?.weightTons ? (
                <div className="flex flex-col border-l-2 border-brass/30 pl-2">
                  <span className="text-[10px] uppercase text-parchment/50">Weight</span>
                  <span className="mt-0.5 text-sm font-bold text-parchment"><bdi dir="ltr">{vehicle.mobility.weightTons.toFixed(1)} t</bdi></span>
                </div>
              ) : null}
            </div>
          )}
            ...
            <CollapsibleSection title={t("modal.sectionArmor")} eyebrow="SURVIVABILITY" defaultOpen>
              <div className="grid grid-cols-1 gap-3 font-mono text-xs sm:grid-cols-2">
                {vehicle.armor.hullFrontMm ? (
                  <ArmorRow label="Hull" front={vehicle.armor.hullFrontMm} side={vehicle.armor.hullSideMm} back={vehicle.armor.hullRearMm} />
                ) : null}
                {vehicle.armor.turretFrontMm ? (
                  <ArmorRow label="Turret" front={vehicle.armor.turretFrontMm} side={vehicle.armor.turretSideMm} back={vehicle.armor.turretRearMm} />
                ) : null}
              </div>
              ...
              <div className="space-y-3">
                {vehicle.firepower.reloadBaseSec ? (
                  <p className="font-mono text-xs text-parchment/70">
                    Reload:{" "}
                    <bdi dir="ltr">{vehicle.firepower.reloadBaseSec}s → {vehicle.firepower.reloadAcedSec}s</bdi>
                  </p>
                ) : null}
                        ...
                        <tr key={i} className="border-b border-hairline/40 text-parchment">
                          ...
                          {["10m", "500m", "1000m", "2000m"].map((r) => (
                            <td key={r} className="py-1 text-end">
                              <bdi dir="ltr">
                                {round.penetration?.find((p: any) => `${p.rangeM}m` === r)?.angle0 ?? round[r] ?? "—"}
                              </bdi>
                            </td>
                          ))}
                        </tr>
              ...
              <div className="grid grid-cols-1 gap-3 font-mono text-xs sm:grid-cols-2">
                {group.entries.map(([key, value]) => (
                  <div key={key} className="flex flex-col border-l-2 border-brass/30 pl-2">
                    ...
                    <span className="mt-0.5 text-sm font-bold text-parchment"><bdi dir="ltr">{formatValue(value)}</bdi></span>
                  </div>
                ))}
              </div>
            </CollapsibleSection>
          ...
          {displayProTips.length > 0 && (
            <CollapsibleSection title={t("modal.sectionProTips")} eyebrow={t("modal.fieldNotes")}>
              <ul className="list-inside list-disc space-y-1 text-start font-body text-sm text-parchment/75">
                {displayProTips.map((tip: string, i: number) => <li key={i}>{tip}</li>)}
              </ul>
            </CollapsibleSection>
          )}
        </div>
...
function ArmorRow({ label, front, side, back }: { label: string; front?: number; side?: number; back?: number }) {
  return (
    <div className="flex flex-col border-l-2 border-brass/30 pl-2">
      <span className="text-[10px] uppercase text-parchment/50">{label}</span>
      <span className="mt-0.5 text-sm font-bold text-parchment">
        <bdi dir="ltr">{front ?? "—"} / {side ?? "—"} / {back ?? "—"} mm</bdi>
      </span>
    </div>
  );
}
...
function BrStat({ label, value }: { label: string; value: number }) {
  return (
    <div className="px-3 py-2.5 text-center">
      ...
      <p className="font-mono text-lg text-brass"><bdi dir="ltr">{value.toFixed(1)}</bdi></p>
    </div>
  );
}
