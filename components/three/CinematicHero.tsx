...
// FIX: Scene3D wraps a react-three-fiber <Canvas>, which touches
// browser-only globals (window, WebGL) that don't exist in the Node.js
// process running `next build`. This page is a "use client" component,
// but Next still does one server-side render pass at build time to
// produce the initial HTML — a static `import Scene3D from "./Scene"`
// would pull it into that pass and fail the build. next/dynamic with
// ssr:false skips Scene3D entirely on the server and only ever mounts
// it in the browser.
