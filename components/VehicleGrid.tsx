...
const GridList: GridComponents["List"] = forwardRef(function GridList(
  ...
  return (
    <div
      ...
      style={style}
      {...props}
      className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5"
    >
      {children}
    </div>
  );
...
export function VehicleGrid({
  ...
  if (isLoading) {
    return (
      <div className="grid grid-cols-2 gap-3 p-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-4 xl:grid-cols-5">
        {Array.from({ length: 12 }).map((_, i) => (
          <div
            key={i}
            className="h-52 animate-pulse rounded-sm border border-hairline bg-panel"
          />
        ))}
      </div>
    );
