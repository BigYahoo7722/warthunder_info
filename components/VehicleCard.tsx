...
export function VehicleCard({
  ...
  return (
    <motion.button
      ...
      className="group flex h-full flex-col overflow-hidden rounded-xl border border-hairline/70 bg-panel/50 text-start backdrop-blur-md transition-colors hover:border-brass/60 hover:bg-panel2/60 focus-visible:border-brass"
    >
      <motion.div
        layoutId={`card-plate-${vehicle.id}`}
        className="relative h-32 w-full shrink-0 overflow-hidden bg-gradient-to-br from-panel2/80 to-ink/80 sm:h-36"
      >
        {hasImage ? (
      ...
      <div className="flex flex-1 flex-col gap-1 p-3">
        <p className="line-clamp-2 min-h-[2.4em] text-start font-body text-[14px] font-medium leading-tight text-parchment">
          {vehicle.name}
        </p>
